...
def ApplyForLeave(request):
    ...
    if (request.method == "POST") and (request.POST.get('leaveOption', '') != ""):
        empleaverequest = EmpLeaveRequest(Emp_ID=employee, Emp_FullName=empMgrDept.Emp_FullName,
                                          ...
                                          End_Date=request.POST['EndDate'], Requested_Days=request.POST['Days'],
                                          Leave_Status="Pending", Emp_Comments=request.POST['Reason'])
        ...
        send_mail('Leave approval request for ' + employeeFullName,
                  'Please access leave portal to approve leave request for ' + employeeFullName + '.\nRequest Type: ' +
                  request.POST['leaveOption'] + '\nReason: ' + request.POST['Reason'] + '\nFrom: ' + request.POST[
                      'BeginDate'] + '\nTo:' + request.POST['EndDate'], employee.Email_Address,
                  [empMgrDept.Manager_Email_Address], fail_silently=False)
        if manager.Designation == 'HR':
            webhook_url = "https://hooks.slack.com/services/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"
            text = 'Leave approval request from ' + employeeFullName +\
                   '. \n Please access leave portal to approve leave request.' + \
                   '\nReason: ' + request.POST['Reason'] + '\nFrom: ' + \
                    request.POST['BeginDate'] + '- To:' + request.POST['EndDate']
            print(text)
    ...
    if (canceldLeave != ""):
        ...
        empleaverequest.save()
        leave_balance = LeaveBalance.objects.get(Emp_No_LeaveBal=empleaverequest.Emp_ID,
                                                 Leave_Type=empleaverequest.Leave_Type)
        leave_balance.Available_Days = leave_balance.Available_Days + empleaverequest.Requested_Days
        leave_balance.save()
        send_mail('Leave cancelled',
                  'Your Leave has been cancelled by ' + empleaverequest.Emp_FullName + ".\nFrom: " + empleaverequest.Begin_Date.strftime(
                      "%Y-%m-%d") + "\nTo: " + empleaverequest.End_Date.strftime("%Y-%m-%d"), employee.Email_Address,
                  [employee.Email_Address, empMgrDept.Manager_Email_Address], fail_silently=False)
...
def ApproveLeave(request):
    ...
    if (approvedLeaves != ''):
        ...
        empleaverequest.save()
        send_mail('Leave approved',
                  'Your Leave has been approved by ' + empleaverequest.Manager_FullName + ".\nFrom: " + empleaverequest.Begin_Date.strftime(
                      "%Y-%m-%d") + "\nTo: " + empleaverequest.End_Date.strftime("%Y-%m-%d"), employee.Email_Address,
                  [employee.Email_Address], fail_silently=False)
    if (declinedLeaves != ''):
        ...
        leave_balance.Available_Days = leave_balance.Available_Days + empleaverequest.Requested_Days
        leave_balance.save()
        send_mail('Leave declined',
                  'Your Leave has been declined by ' + empleaverequest.Manager_FullName + ".\nFrom: " + empleaverequest.Begin_Date.strftime(
                      "%Y-%m-%d") + "\nTo: " + empleaverequest.End_Date.strftime("%Y-%m-%d"), employee.Email_Address,
                  [employee.Email_Address], fail_silently=False)
    ...
    return render(request, 'LMS/ApproveLeave.html', {'employees': employee, 'leavesArray2': leavesArray2,
                                                     'role': userinfo[settings.METADATA_NAMESPACE + 'app_metadata'][
                                                         'role']})
