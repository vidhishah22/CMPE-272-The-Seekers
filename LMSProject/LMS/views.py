...
from .decorators import login_required, manager, hr, employee, hr_or_manager
...
def ApplyForLeave(request):
    ...
    if request.method == "POST":
        empleaverequest = EmpLeaveRequest(Emp_ID=employee, Emp_FullName=empMgrDept.Emp_FullName,
                                          Leave_Type=request.POST['leaveOption'], Manager_Emp_No=manager,
                                          Manager_FullName=empMgrDept.Manager_FullName,
                                          Begin_Date=request.POST['BeginDate'], End_Date=request.POST['EndDate'],
                                          Requested_Days=request.POST['Days'], Leave_Status="Pending",
                                          Emp_Comments=request.POST['Reason'])
    ...
    return render(request, 'LMS/ApplyForLeave.html',
                  {'employees': request, 'leavesArray': leavesArray, 'leave_balance': leave_balance, 'role': userinfo[settings.METADATA_NAMESPACE + 'app_metadata']['role']})
